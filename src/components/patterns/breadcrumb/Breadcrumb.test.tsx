...
describe('Breadcrumb', () => {
	...
	it('renders the last item as the current page', () => {
		...
		expect(
			screen.queryByRole('link', { name: 'Widget' })
	...
	it('renders separators between items', () => {
		...
		expect(container.querySelectorAll('.breadcrumb__separator')).toHaveLength(
			2
		);
	});
	...
	it('renders a single current page without separators', () => {
		...
		expect(
			container.querySelector('.breadcrumb__separator')
		).not.toBeInTheDocument();
	});
	...
	it('accepts a custom aria-label', () => {
		render(<Breadcrumb items={items} aria-label="Page location" />);
